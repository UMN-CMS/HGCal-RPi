...
This contains the legacy Raspberry Pi code.

From the original code, there are two changes.
...
The rdout\_mask set by the user is not used.
It is now automatically set by the cables\_mask that the sync board provides.
This allows easier swapping/addition of readout boards, since the rdout\_mask doesn't have to be set whenever the cables get changed.
