...
int main(int argc, char *argv[])
{
  ...
  fclose(fid);
  char boardID[20]; sprintf(boardID, "_RDOUT%d", board_id);
  ...
  uint64_t p_stamp = 0, this_trig = 0, first_trig = 0;
  ...
  if(AUTOPILOT == 0) {
    for(i = 0; i < maxevents; i = i + 1) {
      ...
      if( !(i % 10) && (access(fEND, R_OK) != -1) ) break;// end if EUDAQ creates a file
	...
	// send put trigger to each ORM
      ...
      if(i == 0) first_trig = (stamp2 << 32) | (stamp1 << 16) | stamp0;
      this_trig = (stamp2 << 32) | (stamp1 << 16) | stamp0 - first_trig;
      trig1 = CTL_get_trig_count1();

      fprintf(stderr, "Trig Number: %04d\n", (int)(i));
      fprintf(ftrig, "%04d\t", (int)(i));
      fprintf(ftrig, "%04d\t", (int)( (trig1 << 16) | trig0));
      fprintf(ftrig, "%04x", (int)(this_trig >> 32)); fprintf(ftrig, "%04x", (int)(this_trig >> 16)); fprintf(ftrig, "%04x\t", (int)(this_trig));
